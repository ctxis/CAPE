...
rule Emotet
{
    ...
    strings:
        ...
        $snippet5 = {8B E5 5D C3 B8 ?? ?? ?? ?? A3 ?? ?? ?? ?? A3 ?? ?? ?? ?? 33 C0 21 05 ?? ?? ?? ?? A3 ?? ?? ?? ?? 39 05 ?? ?? ?? ?? 74 18 40 A3 ?? ?? ?? ?? 83 3C C5 ?? ?? ?? ?? 00 75 F0 51 E8 ?? ?? ?? ?? 59 C3}        $c2list = {?? ?? ?? ?? (50 00|2F 10|BB 01|90 1F|A8 1B) 00 00 ?? ?? ?? ?? (50 00|2F 10|BB 01|90 1F|A8 1B) 00 00 ?? ?? ?? ?? (50 00|2F 10|BB 01|90 1F|A8 1B) 00 00 ?? ?? ?? ?? (50 00|2F 10|BB 01|90 1F|A8 1B) 00 00}
    condition:
        //check for MZ Signature at offset 0
        uint16(0) == 0x5A4D and (($snippet1) and ($snippet2) and ($c2list)) or ($snippet3) or ($snippet4) or ($snippet5)
}
